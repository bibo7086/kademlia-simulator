...
/**
 * Provides static methods to obtain the package-qualified class name of a class, given just the
 * non-qualified name, and to obtain the non-qualified name, given the package-qualified class name.
 *
 * <p>Inspired from some code written by David Postill (david@postill.org.uk) (found in
...
class ClassFinder {
  ...
  /**
   * The number of directories that have been touched by the search. This does not include
   * directories in jar files.
   */
  ...
  static {
    ...
    if (visitedDirs >= maxDirs) {
      System.err.println(
          "Configuration: some directories in your "
  ...
  /** Returns the non-qualified name of a class, removing all the package information. */
  ...
  /**
   * Returns the package-qualified name associated to the specified non-qualified name, if exists.
   * Otherwise it returns null.
   *
   * <p>Only classes reachable from the classpath defined by the "java.class.path" property are
   * considered. Jar files and directories are both parsed. If multiple classes with the same name
   * but different fully-qualified names are present, a comma-separated list of fully-qualified
   * class names is returned.
   *
  ...
  /**
   * Finds all the classes reachable from the current classpath; for each of them, inserts an
   * association (name, fully-qualified name) in the specified map. Both names are String objects.
   *
   * <p>Only classes reachable from the classpath defined by the "java.class.path" property are
   * considered. Jar files and directories are both parsed. If multiple classes with the same name
   * but different fully-qualified names are present, they are inserted in the map as associations
   * (name, comma-separated list of fully-qualified names).
   *
   * @param map
   * @throws IOException
   */
   ...
   * @throws IOException
   */
  private static void findClassInPathDir(Map<String, String> map, String pathElement, File pathFile)
      throws IOException {
    visitedDirs++;
  ...
  /** Translates a class file name in a class name using the specified file separator. */
