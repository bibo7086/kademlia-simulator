...
public class GeneralNode implements Node {
  ...
  /**
   * The current index of this node in the node list of the {@link Network}. It can change any time.
   * This is necessary to allow the implementation of efficient graph algorithms.
   */
  private int index;
  ...
  /**
   * The ID of the node. It should be final, however it can't be final because clone must be able to
   * set it.
   */
  private long ID;
  ...
  public void setFailState(int failState) {
    ...
    switch (failState) {
      ...
      case DEAD:
        ...
        failstate = DEAD;
        for (int i = 0; i < protocol.length; ++i)
          if (protocol[i] instanceof Cleanable) ((Cleanable) protocol[i]).onKill();
        break;
      case DOWN:
        failstate = DOWN;
        break;
